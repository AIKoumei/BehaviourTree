...
------------------------------------------------
-- 执行方法
function BehaviourTreeCompositeSelectorNode:Run(entity)
    local childs = self:GetChilds()
    local data = BehaviourTree.GetBlackboardData(entity)
    local running = data.running_node or childs[1]
    -- 运行节点
    return running:run(entity)
end

-- 接受运行结果
-- 当节点运行完毕，返回给父节点的时候，调用该方法，更新下次运行的节点
function BehaviourTreeCompositeSelectorNode:ReciveRunResult(entity, running_node, result)
    ...
    elseif result == BehaviourTree.RunTimeResultEnum.Failed then
        if running_node == childs[#childs] then
            self:GetParent():ReciveRunResult(entity, self, result)
        else
